  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {mode === "search" && !selected && (
        <>
          <div style={{ fontSize: 11, color: t.textMuted, lineHeight: 1.6 }}>Search by game name. Results from HowLongToBeat.</div>
          ...
          {searchResults.length > 0 && (
            <div style={{ border: `1px solid ${t.border}`, background: t.bgSurface, maxHeight: 260, overflowY: "auto", display: "flex", flexDirection: "column", gap: 1 }}>
              {searchResults.map(r => (
                ...
                >
                  {r.imageUrl
                    ? <img src={r.imageUrl} alt="" style={{ width: 28, height: 36, objectFit: "cover", flexShrink: 0, border: `1px solid ${t.borderSubtle}` }} />
                    : <div style={{ width: 28, height: 36, background: t.bgElevated, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, color: t.textMuted }}>?</div>
                  }
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: "Rajdhani, sans-serif", fontSize: 13, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", lineHeight: 1.2 }}>{r.title}</div>
                    <div style={{ color: t.textSecondary, fontSize: 10, display: "flex", gap: 8, marginTop: 2, flexWrap: "wrap" }}>
                      ...
                      {r.platforms[0] && <span style={{ color: t.textMuted }}>{r.platforms[0]}</span>}
                    </div>
                  </div>
                </button>
              ))}
          ...
          {!searching && query.trim().length >= 2 && searchResults.length === 0 && !error && (
            <div style={{ color: t.textMuted, fontSize: 11, textAlign: "center", padding: "12px 0" }}>No results found</div>
          )}
        </>
      )}
      ...
      {mode === "url" && !selected && (
        <>
          <div style={{ fontSize: 11, color: t.textSecondary, lineHeight: 1.7, padding: "6px 8px", background: t.bgInput, border: `1px solid ${t.borderSubtle}` }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <input value={url} onChange={e => { setUrl(e.target.value); setError(null); }} onKeyDown={e => e.key === "Enter" && handleFetchUrl()}
              ...
              style={{ ...inputBase, flex: 1, padding: "7px 10px", fontSize: 11, clipPath: "polygon(0 0, calc(100% - 5px) 0, 100% 5px, 100% 100%, 0 100%)", minWidth: 0 }}
              onFocus={e => (e.target.style.borderColor = t.accent)}
              onBlur={e => (e.target.style.borderColor = t.border)}
            />
      ...
      {selected && (
        <div style={{
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
            {selected.imageUrl
              ? <img src={selected.imageUrl} alt="" style={{ width: 44, height: 58, objectFit: "cover", flexShrink: 0, border: `1px solid ${t.border}` }} />
              : <div style={{ width: 44, height: 58, background: t.bgSurface, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>🎮</div>
            }
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {selected.platforms.length > 0 && <div style={{ fontSize: 10, color: t.textSecondary }}>{selected.platforms.slice(0, 3).join(" · ")}</div>}
              {selected.developer && <div style={{ fontSize: 10, color: t.textMuted, marginTop: 1 }}>{selected.developer}</div>}
            </div>
            <button onClick={clearSelected} style={{ background: "none", border: "none", color: t.textSecondary, cursor: "pointer", fontSize: 14, padding: 2, flexShrink: 0 }}>✕</button>
          </div>

          {/* Completion type */}
          <div>
            <div style={{ color: t.textSecondary, fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 6 }}>Completion Type</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
              {(["main", "main_sides", "completionist", "average"] as CompletionType[]).map(ct => {
                ...
                return (
                  ...
                  >
                    <div style={{ fontSize: 9, opacity: 0.7 }}>{COMPLETION_LABELS[ct]}</div>
              ...
              <button onClick={() => setCompletionType("custom")}
                style={{
                  ...
                  cursor: "pointer", fontSize: 11, fontFamily: "DM Mono, monospace", textAlign: "left",
                  clipPath: "polygon(0 0, calc(100% - 4px) 0, 100% 4px, 100% 100%, 0 100%)",
                  gridColumn: hasAnyHltb ? "auto" : "1 / -1",
                }}
              >
                <div style={{ fontSize: 9, opacity: 0.7 }}>Custom</div>
                <div style={{ fontFamily: "Rajdhani, sans-serif", fontSize: 14, fontWeight: 700 }}>{customHours ? `${customHours}h` : "—"}</div>
              </button>
            </div>

            {completionType === "custom" && (
              <div style={{ marginTop: 8 }}>
                <div style={{ color: t.textSecondary, fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>Custom Hours</div>
          ...
          {/* Start date */}
          <div>
            <div style={{ color: t.textSecondary, fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 6 }}>Start Date</div>
            <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)}
              style={{ width: "100%", ...inputBase, background: t.bgInput, padding: "6px 10px", fontSize: 12, colorScheme: t.mode as any }}
            />
          </div>

          <div style={{ fontSize: 11, color: t.textSecondary, padding: "4px 8px", background: t.bgInput, border: `1px solid ${t.borderSubtle}` }}>
...
function ErrorBox({ msg, sub }: { msg: string; sub?: string }) {
  const { theme: t } = useTheme();
  return (
    <div style={{ color: t.danger, fontSize: 11, padding: "6px 8px", background: `${t.danger}12`, border: `1px solid ${t.danger}30`, lineHeight: 1.5 }}>
      {msg}
